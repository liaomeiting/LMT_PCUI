...
typedef struct{
    ...
    void (* delay)(void);
    uint8_t ctrl_det_en;		//是否使能总线控制探测
    uint16_t wait_overtime_cnt;
    uint32_t ctrl_overtime_cnt;	//启用ctrl_det_en，才有效	
}i2c_adapter;
...
int P_I2C_Start(i2c_adapter *adap);
int P_I2C_Stop(i2c_adapter *adap);
int P_I2C_SendByte(i2c_adapter *adap,uint8_t dat);
int P_I2C_WaitAck(i2c_adapter *adap);
uint8_t P_I2C_ReadByte(i2c_adapter *adap);	//弃用
int P_I2C_ReadByte2(i2c_adapter *adap,uint8_t *data);	
int P_I2C_ReplyAck(i2c_adapter *adap,uint8_t ack);

/*---iic接口---*/

//复位总线
int P_I2C_Restart(i2c_adapter *adap);
...
//单字节掩码写
int P_I2C_WriteRegMask(i2c_adapter *adap,uint8_t slave_addr,uint8_t reg,uint8_t mask,uint8_t dat);
