...
* 作    者 ：Optics group
* 修改日期 ：2022-2-8
* 修改描述 ：a.增加CA310功能函数结构体测量色温功能
*            b.修复CA310一拖多测量色温异常问题
*
* 版    本 ：V1.13
* 作    者 ：Optics group
* 修改日期 ：2022-12-30
...
* 作    者 ：Optics group
* 修改日期 ：2022-12-02
* 修改描述 ：a.修复CA410获取同步频率bug
*
* 版    本 ：V1.10
* 作    者 ：Optics group
* 修改日期 ：2022-11-25
* 修改描述 ：a.修改CA410零校准超时时间,适配CL4零校准时间过长
*            b.增加CL4使用310模式测量flicker百分值功能
...
/**
  * 功能描述 : 查询探头类型
  * 输入参数 : 1. PN -- 默认为1
  *              [CA-310]多探头输出时,PN = (1..5)
  *              [CA-410]保留参数
  * 输出参数 : 1.  type -- 镜头类型  固定16位，少于16字符左对齐
  * 返 回 值 : = 0 -- 没有此功能, = 1 -- 查询成功, < 0 -- 查询失败
  */
int COMCA410_GetType(Ca310Interface *ca410,uint8_t PN,char* type);
...
/**
  * 功能描述 : 查询CL4 310模式的flicker百分值
  * 输入参数 : 1. ca410 -- Ca310Interface类型结构体
  *            2. PN -- 保留参数,值无效
  * 输出参数 : 1. FMA -- flick值
  * 返 回 值 : = 1 -- 查询成功,  < 0 -- 查询失败
  * 注意事项 : 仅适用CL4,禁止短时间内连续调用
  */
int COMCA410_GetCA310ModeFMA(Ca310Interface *ca410, uint8_t PN, float* FMA);
...
/**
  ...
  *               9 - CL4 CA310 flicker
  * 返 回 值 : = 1 -- 设置成功, = 0 -- 无此功能, < 0 -- 设置失败
  */
int COMCA410_SetMeasureMode(Ca310Interface *ca410, uint8_t PN, int mode);
...
/**
  ...
  * 返 回 值 : = 1 -- 查询成功,  < 0 -- 查询失败
  * 注意事项 : 必须先配置好镜头模式,才能获取正确的值
  */
int COMCA410_ContinuousMeasure(Ca310Interface *ca410, uint8_t PN, int *mode, float (*var)[4]);
...
/**
  ...
  *            3. RGBW_calVal -- 红绿蓝白画面的xylv目标数值	
  *            4. mode -- 0测量白画面,1测量红画面,2测量绿画面,3测量蓝画面,4执行校准
  * 返 回 值 : = 1 -- 设置成功,  < 0 -- 设置失败
  * 注意事项 :测量画面顺序为白->红->绿->蓝
  */
int COMCA410_SetCal(Ca310Interface *ca410, uint8_t PN, int CH, double (*RGBW_calVal)[3], int mode);

/********************************************************************************************
  * 函数定义 : CA410_GetJEITADB
  * 功能描述 : 查询JEITA下的所有频率数据
  * 输入参数 : 1. PN -- 保留参数,值无效
  *            2. Hz -- CA410探头JEITA测量模式下的步进频率 设置范围：0.07Hz~0.09Hz(间隔0.01Hz)  0.1Hz~0.9Hz(间隔0.1Hz)   1Hz~10Hz(间隔1Hz)
  * 输出参数 : 1. num -- JEITA测量模式下的数据大小
  *			  2.var -- JEITA测量模式下的数据缓存区  其中var[0]最小频率  var[1]最大频率
  *           3.JEITA_DB --  JEITA闪烁值
  *		 	  4.JEITA_fre -- JEITA闪烁频率
  * 返 回 值 : 查询状态
  *           1. = 1 -- 设置成功,  < 0 -- 设置失败
********************************************************************************************/
int CA410_GetJEITADB(Ca310Interface *ca410,uint8_t PN,float Hz,int *num ,float *var,float *JEITA_DB,float *JEITA_fre);
...
/**
  ...
  *			   2. fre --  JEITA模式下的采样频率值
  * 返 回 值 : = 1 -- 查询成功  2. < 0 -- 查询失败
  */
int COMCA410_GetJEITASamples(Ca310Interface *ca410, uint8_t PN, float* step, int* fre);
