...
#if PCUI_FLAG
		/* 获取指令,读取对应指令数据 */
		// if(pcui_getCmd(string_buffer) >= 0)
		// {
        //     _DEBUG("%s\r\n", string_buffer);
		// 	if(pcui_Scan(string_buffer,"otpstart\r\n")>0)
		// 	{
		// 		if ((auto_switch_mode == 0) && (display_on == 1))
		// 		{
		// 			KEY_SetLED(KEY_OTP, KEYLED_ON); // OTP按键灯亮
		// 			KEY_Enter();
        //             sprintf(wdata, "#000P%04dT1200!",location0);
        //             BUSServo_CmdPassthrough111(Servo_CN1,wdata,strlen(wdata),rdata,128);                                                   

		// 			KEY_SetLED(KEY_OTP, KEYLED_OFF);				// 执行OTP烧录操作
		// 		} 
		// 	}
		// }
        User_Event();
