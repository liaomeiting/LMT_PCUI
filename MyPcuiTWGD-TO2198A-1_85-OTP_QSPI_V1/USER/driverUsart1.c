...
void USART1_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
    {
        ...
        // 处理FIFO（如果存在）
        if(Usart1_Driver.fifo != NULL)
            myFIFOWrite(Usart1_Driver.fifo, &Res, 1);
        
        // 处理单字节回调（如果存在）
        if(Usart1_Driver.single != NULL)
            Usart1_Driver.single(&Usart1_Driver, &Res, 1);

        // 软件缓冲区处理（添加边界检查）
		if (Res == '\r' || Res == '\n') {
            if (USART1_RX_CNT > 0) {
                USART1_RX_BUF[USART1_RX_CNT] = '\0'; // 字符串终止符
                cmdReady = 1;            // 标记命令已就绪
            }
            USART1_RX_CNT = 0; // 重置索引
        } 
        else if (USART1_RX_CNT < USART1_RX_BUF_SIZE - 1) {
            USART1_RX_BUF[USART1_RX_CNT++] = Res; // 存储数据
        }
        // 检查并处理溢出错误
        if(USART_GetFlagStatus(USART1, USART_FLAG_ORE) == SET)
        {
            USART_ReceiveData(USART1); // 读取数据清除ORE标志
            // 可以记录错误或调用错误处理回调
        }
    }}
