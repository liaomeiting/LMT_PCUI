...
BusFilterStruct sg_bus_filter[BUS_MAX_FILTER]; //14个滤波器的状态
...
//	CAN_InitStructure.CAN_NART=ENABLE;		    //禁止报文自动传送 //
    ...
    if(bus_type == 0)
    {
        ...
        CAN_InitStructure.CAN_BS1 = CAN_BS1_5tq;//CAN_BS1_6tq;//5;  //Tbs1=tbs1+1个时间单位CAN_BS1_1tq ~CAN_BS1_16tq
        CAN_InitStructure.CAN_BS2 = CAN_BS2_1tq;//0;  Tbs2=tbs2+1个时间单位CAN_BS2_1tq ~	CAN_BS2_8tq
        CAN_InitStructure.CAN_Prescaler = 21;  //分频系数17;    
    }
    else if(bus_type == 1)
	{
        //500k
        ...
        CAN_InitStructure.CAN_BS1 = CAN_BS1_5tq;//CAN_BS1_6tq;//5;  //Tbs1=tbs1+1个时间单位CAN_BS1_1tq ~CAN_BS1_16tq
        CAN_InitStructure.CAN_BS2 = CAN_BS2_1tq;//0;  Tbs2=tbs2+1个时间单位CAN_BS2_1tq ~	CAN_BS2_8tq
        CAN_InitStructure.CAN_Prescaler = 12;  //分频系数11;    
	}
...
static u8 CANBus_AddRxMdl(unsigned int bus_type, BusFilterStruct* filt_struct)
{
    ...
    {
        CAN_FilterInitStructure.CAN_FilterIdHigh=(0x1<<13)|  //頭
...
static u8 CANBus_SendMsg(unsigned int bus_type, BusTxMsgStruct* bus_tx_msg)
{
	...
	while((CAN_TransmitStatus(CAN1, mbox)!=CANTXOK)&&(i<0XfFFF))i++;	//等待发送结束
	if(i>=0XfFFF)
	{
		return BUS_TXFAILED;
	}
...
/********************************************************************************
*函數名：USB_LP_CAN1_RX0_IRQHandler
*描述  ：CAN1接收中断
*参数  ：无
*返回  ：无     
********************************************************************************/
