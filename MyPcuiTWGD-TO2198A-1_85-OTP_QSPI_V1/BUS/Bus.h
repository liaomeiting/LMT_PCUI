...
6. BusTxMsgStruct.dlc      - 發送的数据长度 0~8
7. BusTxMsgStruct.data[8]  - 發送的数据
*/
typedef struct
{
	u32 resev1:32;
...
typedef struct
{
	...
	u8 src_type; 	//源模块（即發送端）类型
	u8 src_id;   	//源模块（即發送端）ID
	u8 filt_src;    //使能过滤 發送端的类型和ID
...
#define BUS_CFL2    	16 //新镜头//#define BUS_CE48CORE    16 //ce48核心板
...
/*mBUS發送邮箱状态*/
#define BUS_NO_MB 0x04 //Bus没有可用的邮箱来發送信息
#define BUS_TXFAILED 0X05 //發送失败
...
6. mBusTxMsgStruct.dlc      - 發送的数据长度 1~8
7. mBusTxMsgStruct.data[8]  - 發送的数据
*/
typedef struct
{
	u32 resev1:32;
...
#define MBUS_CCD         5 //镜頭
#define MBUS_RS232       6 //mBus转串口(RS232)
#define MBUS_MOTOR       7 //电机驱动

/*mBUS發送邮箱状态*/
#define MBUS_NO_MB 0x04 //mBus没有可用的邮箱来發送信息
#define MBUS_TXFAILED 0X05 //發送失败
...
*返回  ：發送消息用到的邮箱号， 0~2
*        =MBUS_NO_MB 说明没有空邮箱，發送失败
*        =MBUS_TXFAILED 
*/
u8 mBus_SendMsg(mBusTxMsgStruct* mbus_tx_msg);

/*
*函數名：mBus_DisableRxIT
*描述  ：禁止mBus 接收中断
*参数  ：无
*返回  ：无     
*/
